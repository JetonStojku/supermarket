...
class Fatura(models.Model):
    ...
    monedha = fields.Many2one(comodel_name='monedha.monedha', string='Monedha', default=1, required=True)  # default leku, first record
    ...
    def _llogarit_kursin(self):
        self.kursi = 1 / self.monedha.kursi_kembimit if self.monedha else 1
    ...
    def create(self, values):
        return super(Fatura, self).create(values)

    @api.multi
    def write(self, vals):
        for fature in self:
            return super(Fatura, fature).write(vals)
