...
class Shporta(models.Model):
    ...
    fatura = fields.Many2one(comodel_name='fatura', string='Fatura', ondelete="cascade")
    produkti = fields.Many2one(comodel_name='produkti', string='Produkti')
    ...
    def create(self, values):
        if self._context.get('inherited_mag'):
            return super(Shporta, self).create(values)
        else:
            res = super(Shporta, self).create(values)
    ...
    def write(self, vals):
        for shporte in self:
            if shporte._context.get('inherited_mag'):
                return super(Shporta, shporte).write(vals)
            else:
    ...
    def unlink(self):
        for shporte in self:
            if shporte._context.get('inherited_mag'):
                return super(Shporta, shporte).unlink()
            else:
