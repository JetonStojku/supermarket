...
class Fatura(models.Model):
    ...
    klienti = fields.Many2one(comodel_name='klienti', string='Klienti', default=0)
    ...
    shporta_ids = fields.One2many(comodel_name='shporta', inverse_name='fatura', string='Shporta_ids')

    @api.multi
    @api.constrains('per_te_paguar', 'dhene_ne_dore')
    def _check_reconcile(self):
        for fatura in self:
            if fatura.dhene_ne_dore < 0 or fatura.dhene_ne_dore < fatura.per_te_paguar:
                raise ValidationError('Vlera e paguar nuk është e mjaftueshme')

    @api.multi
    @api.depends('shporta_ids')
    def _llogarit_totalin(self):
        for obj in self:
            obj.totali = sum(obj.shporta_ids.mapped('totali'))

    @api.multi
    @api.depends('dhene_ne_dore', 'per_te_paguar')
    def _llogarit_kusurin(self):
        for obj in self:
            obj.kusuri = obj.dhene_ne_dore - obj.per_te_paguar
    ...
    def create(self, values):
            res = super(Fatura, self).create(values)
    ...
    def write(self, vals):
        for fature in self:
            ...
            res = super(Fatura, fature).write(vals)
    ...
    def unlink(self):
        if self._context.get('inherited_mag'):
            return super(Fatura, self).unlink()
        else:
            for fature in self:
                ...
                if fature.menyra_pageses == 'cash':
                    ...
                    fature.klienti.piket_shpenzuar -= fature.pike_paguar
                return super(Fatura, fature).unlink()
